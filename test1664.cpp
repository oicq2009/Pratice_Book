...
namespace NiuNiu 
{
	...
	public:
		Stack() { _top = -1; }                       /* 默认构造函数 */
		Stack(int n) { create_stack(n); }

		int create_stack(int n) {                    /* 创建长度为n的栈 */
            if (n <= 0) {
			    return 0;
		    }
            _array = new T[n];
        ...
        int is_empty() { return _top == -1; }        /* 判断栈是否为空 */
		int is_full() { return _top >= _size - 1; }  /* 判断栈是否为满 */
		T top() { return _array[_top]; }             /* 返回栈顶元素 */
		...
		friend int is_operator(char& ch);  // 判断是否为运算符
	...
	int get() {
		...
		a = getchar();
		while (a != '#' || OPTR.top() != '#') {
			if (!is_operator(a))   //不是运算符，即为操作数，操作数入栈
			...
			c = a;
			a = getchar();                         //继续读取字符
            /*若连续读取字符均为数字，则乘以位权得到多位数*/
			while (!is_operator(a) && !is_operator(c)) {
				OPND.pop(m);
				m = m * 10 + a - 48;
				OPND.push(m);
				c = a;
